...
voca = list(set(cleaned_text.split()))
# print(len(voca))
# print(voca)
for i in range(len(voca)):
    if voca[i].startswith("'"):
        if voca[i] == "'tis":
            voca[i] == voca[i]
        elif voca[i] == "'em":
            voca[i] == voca[i]
        else:
            voca[i] = voca[i][1:len(voca[i])]  # 그 외는 모두 ' 생략
    elif voca[i].endswith("'"):
        voca[i] == voca[i][0:len(voca[i]) - 1]
    elif voca[i] == "i, ii, iii, iv, v, vi, vii, viii, ix, x, xi, xii":
        voca[i] == ''

all = " ".join(voca)
# print(all)
result_voca = all.split()
result_voca.sort()
# print(result_voca)
print(len(set(result_voca)))
